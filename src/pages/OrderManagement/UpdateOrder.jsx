...
const UpdateOrder = ({ visible, onClose, onSuccess, orderData }) => {
  ...
  const getStatusIcon = (statusName) => {
    const iconMap = {
      pending: <ClockCircleOutlined style={{ color: "#faad14" }} />,          // vàng
      processing: <ShoppingCartOutlined style={{ color: "#1890ff" }} />,    // xanh nước biển
      delivered: <CheckCircleOutlined style={{ color: "#52c41a" }} />,      // xanh lá
  ...
  const getStatusColor = (statusName) => {
    const colorMap = {
      pending: "#faad14",          // vàng
      processing: "#1890ff",      // xanh nước biển
  ...
  function getNextStatuses(currentStatus) {
    const statusFlow = {
      pending: ["confirmed", "cancelled"],  // Khách đặt xong là chờ xác nhận --> xác nhận hoặc hủy
      confirmed: ["processing", "cancelled"], // Đã xác nhận --> chuyển đang xử lý hoặc hủy
      processing: ["shipped", "cancelled"],   // Đang xử lý --> sang đang giao hoặc hủy
      shipped: ["delivered", "cancelled"],    // Đang giao --> đã giao hoặc hủy
      delivered: [],                             // Không chuyển đi đâu nữa
