// src/pages/staff/StaffUpdateProduct.jsx
...
const StaffUpdateProduct = ({ visible, onClose, onSuccess, productData, categories = [] }) => {
  ...
  useEffect(() => {
    if (visible && productData) {
      ...
      // Khôi phục ảnh hiện tại (nếu có) vào fileList để hiển thị
      if (productData.image && !fileList.length) {
        setFileList([
          {
            uid: "-1",
            name: "current-image",
  ...
  }, [visible, productData, form, fileList.length]);
          ...
          <div style={{ padding: "8px 0" }}>
            <Space direction="vertical" size="large" style={{ width: "100%" }}>
              <div style={{ textAlign: "center" }}>
                ...
                <Title level={3} style={customStyles.title}>Cập nhật Sản phẩm (Nhân viên)</Title>
              ...
              <Form form={form} layout="vertical" onFinish={handleFinish} size="large">
                <Form.Item label={<Space><AppstoreOutlined style={{ color: "#13C2C2" }} /><span style={customStyles.label}>Danh mục sản phẩm</span></Space>} name="category_id" rules={[{ required: true, message: "Vui lòng chọn danh mục!" }]}>
                  <Select placeholder="Chọn danh mục" style={customStyles.input}>
                ...
                <Form.Item label={<Space><ShoppingOutlined style={{ color: "#13C2C2" }} /><span style={customStyles.label}>Tên sản phẩm</span></Space>} name="name" rules={[{ required: true, message: "Vui lòng nhập tên!" }]}>
                  <Input placeholder="Nhập tên sản phẩm" style={customStyles.input} maxLength={200} showCount />
                </Form.Item>

                <Form.Item label={<Space><DollarOutlined style={{ color: "#13C2C2" }} /><span style={customStyles.label}>Giá sản phẩm</span></Space>} name="price" rules={[{ required: true, message: "Vui lòng nhập giá!" }]}>
                  <InputNumber min={0} max={999999999} style={{ width: "100%", ...customStyles.input }} placeholder="Nhập giá" formatter={(v) => `${v}`.replace(/\B(?=(\d{3})+(?!\d))/g, ",")} parser={(v) => v.replace(/\$\s?|(,*)/g, "")} />
                </Form.Item>
                ...
                <Form.Item label={<span style={customStyles.label}>Hình ảnh sản phẩm</span>}>
                  <Upload 
                  ...
                  >
                    {fileList.length < 1 && (
                      <div style={{ padding: "20px 0" }}>
                        ...
                        <div style={{ marginTop: 8, color: "#13C2C2", fontWeight: 500, fontSize: 14 }}>Tải ảnh mới</div>
                ...
                <Form.Item label={<span style={customStyles.label}>Trạng thái hiển thị</span>} name="status" valuePropName="checked">
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <Switch 
                      ...
                      onChange={(checked) => { setSwitchValue(checked); form.setFieldsValue({ status: checked }); }} 
                      checked={switchValue} 
                    />
                    <Text style={{ color: "#666", fontSize: 14 }}>
                      {switchValue ? "Sản phẩm đang hiển thị" : "Sản phẩm đang ẩn"}
                    </Text>
                  </div>
                </Form.Item>
                ...
                <Form.Item style={{ marginBottom: 0 }}>
                  <Space style={{ width: "100%", justifyContent: "space-between" }}>
                    ...
                    <Button type="primary" htmlType="submit" loading={isSubmitting} icon={<EditOutlined />} size="large" disabled={isSubmitting} style={{ ...customStyles.primaryButton, minWidth: 140 }}>
                      Lưu thay đổi
                    </Button>
                  </Space>
                </Form.Item>
