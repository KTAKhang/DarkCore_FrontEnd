// src/pages/staff/StaffCreateProduct.jsx
...
const StaffCreateProduct = ({ visible, onClose, onSuccess, categories = [] }) => {
    ...
    // StaffCreateProduct.jsx
    const activeCategories = categories.filter((c) => c.status === true); // ← CHỈ ACTIVE
    ...
    const handleFinish = async (values) => {
        ...
        const images = fileList.map(f => f?.originFileObj).filter(f => f instanceof File);

        console.log("Form values:", values); // ← KIỂM TRA category_id
        console.log("category_id from form:", values.category_id);

        const payload = {
            name: values.name?.trim(),
            category_id: values.category_id, // ← PHẢI LẤN TỪ values
            price: values.price,
                    ...
                    <div style={{ padding: "8px 0" }}>
                        <Space direction="vertical" size="large" style={{ width: "100%" }}>
                            <div style={{ textAlign: "center" }}>
                                ...
                                <Title level={3} style={styles.title}>Tạo Sản phẩm Mới (Nhân viên)</Title>
                                <Text type="secondary" style={{ fontSize: 14 }}>Thêm sản phẩm mới vào hệ thống</Text>
                            </div>
                            ...
                            <Form form={form} layout="vertical" onFinish={handleFinish} initialValues={{ quantity: 1, price: 1000, status: true }} size="large">
                                <Form.Item
                                    label={<Space><AppstoreOutlined style={{ color: "#13C2C2" }} /><span style={styles.label}>Danh mục sản phẩm</span></Space>}
                                    name="category_id" // ← PHẢI LÀ category_id
                                    rules={[{ required: true, message: "Vui lòng chọn danh mục!" }]}
                                >
                                    <Select placeholder="Chọn danh mục" style={styles.input}>
                                ...
                                <Form.Item label={<Space><ShoppingOutlined style={{ color: "#13C2C2" }} /><span style={styles.label}>Tên sản phẩm</span></Space>} name="name" rules={[{ required: true, message: "Vui lòng nhập tên!" }]}>
                                    <Input placeholder="Nhập tên sản phẩm" style={styles.input} maxLength={200} showCount />
                                </Form.Item>

                                <Form.Item label={<Space><DollarOutlined style={{ color: "#13C2C2" }} /><span style={styles.label}>Giá sản phẩm</span></Space>} name="price" rules={[{ required: true, message: "Vui lòng nhập giá!" }]}>
                                    <InputNumber min={0} max={999999999} style={{ width: "100%", ...styles.input }} placeholder="Nhập giá" formatter={(v) => `${v}`.replace(/\B(?=(\d{3})+(?!\d))/g, ",")} parser={(v) => v.replace(/\$\s?|(,*)/g, "")} />
                                </Form.Item>
                                ...
                                <Form.Item label={<span style={styles.label}>Hình ảnh sản phẩm</span>} rules={[{ required: true, message: "Vui lòng chọn hình ảnh!" }]}>
                                    <Upload
